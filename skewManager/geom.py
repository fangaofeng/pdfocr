...
def rect(lefttop, rightbottom):
    ...
    if lefttop[1] >= rightbottom[1]:
        raise ValueError(
            'lefttop[1] must be smaller than rightbottom[1] to form a rectangle')
...
def find_from_to_xy(line, img_w, img_h):
    ...
    hhomax = img_w+img_h
    # y = ax + b
    x1, y1 = int(x0 + hhomax*(-b)), int(y0 + hhomax*(a))
    x2, y2 = int(y0 - hhomax*(-b)), int(y0 - hhomax*(a))
    """
    # boundary check
    x1 = x1 if x1 <= img_w else x1 if x1 >= 0 else 0
    x2 = x2 if x2 <= img_w else x2 if x2 >= 0 else 0
    y1 = y1 if y1 <= img_h else y1 if y1 >= int(img_h/2) else int(img_h/2)
    y2 = y2 if y2 <= img_h else img_h if y2 >= int(img_h/2) else int(img_h/2)
    """
    return x1, y1, x2, y2
...
def project_polarcoord_lines(lines, img_w, img_h):
    ...
    for i, (rho, theta) in enumerate(lines):
        ...
        hhomax = img_w+img_h
        # y = ax + b
        x1, y1 = int(x0 + hhomax*(-sin_theta)), int(y0 + hhomax*(cos_theta))
        x2, y2 = int(y0 - hhomax*(-sin_theta)), int(y0 - hhomax*(cos_theta))
        """
        a = np.cos(theta)
        b = np.sin(theta)
        x0 = a*rho
